...
@section('main')
    <div class="container-fluid">
        ...
        <form class="row" action="{{ route('admin.jurisprudence.lawyers.update', $lawyer->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="col-md-6">
                <div class="pt-3">
                    <label for="name" class="form-label">Имя</label>
                    <input required id="name" name="name" class="form-control" type="text" value="{{ old('name', $lawyer->name) }}">
                </div>
                <div class="pt-3">
                    <label for="surname" class="form-label">Фамилия</label>
                    <input required id="surname" name="surname" class="form-control" type="text" value="{{ old('surname', $lawyer->surname) }}">
                </div>
                <div class="pt-3">
                    <label for="phone" class="form-label">Телефон</label>
                    <input required id="phone" name="phone" class="form-control" type="text" value="{{ old('phone', $lawyer->phone) }}" placeholder="+7 (999) 999 99-99">
                </div>
                <div class="pt-3">
                    <label for="profession" class="form-label">Профессия</label>
                    <input required id="profession" name="profession" class="form-control" type="text" value="{{ old('profession', $lawyer->profession) }}">
                </div>
                <div class="pt-3">
            ...
            <div class="col-md-6">
                <div class="pt-3">
                    ...
                    <input class="form-control" name="base_price" id="base_price" type="number" min="1" value="{{ old('base_price', $lawyer->base_price) }}">
                </div>
                <div class="pt-3">
                    <label for="experience" class="form-label">Опыт</label>
                    <input class="form-control" name="experience" id="experience" type="number" min="0" value="{{ old('experience', $lawyer->experience) }}">
                </div>
            </div>
        </form>
    </div>
