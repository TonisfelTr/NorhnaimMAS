...
@section('title', 'Результаты теста ' . ($test->name ?? 'Тест'))
@section('assets')
    <style>
        :root {
    ...
    <div class="test-result-page">
        <div class="test-result-toolbar">
            <div class="test-result-toolbar__inner">
                <div class="test-result-toolbar__title">Результаты тестирования</div>

                <div class="test-result-actions">
                    <button type="button"
                            class="test-result-btn test-result-btn--secondary"
        ...
        <div class="test-result-container">
                ...
                <div class="test-result-hero__row">
                    <div>
                        ...
                        <div class="test-result-meta">
                            ...
                            <span>Начало: {{ optional($session->created_at)->format('d.m.Y H:i') ?? '—' }}</span>
                            @if($session->completed_at)
                                <span>Завершение: {{ optional($session->completed_at)->format('d.m.Y H:i') }}</span>
                            @endif
                        </div>
                    </div>
            ...
            <section class="test-result-stats">
                ...
                <div class="test-result-stat test-result-stat--warning">
                    ...
                    <div class="test-result-stat__hint">Без сохранённого результата</div>
                </div>

                <div class="test-result-stat test-result-stat--neutral">
                    <div class="test-result-stat__label">
                        {{ $isQuestionnaire ? 'Общая оценка' : 'Интерпретация' }}
                    </div>

                    <div class="test-result-stat__value"
                         style="font-size: {{ $isQuestionnaire ? '29px' : '19px' }}; line-height: 1.25;">
                        @if($isQuestionnaire)
                            {{ $overviewScore }}{{ $overviewMax ? ' / ' . $overviewMax : '' }}
                        @else
                            Врачом
                        @endif
                    </div>

                    <div class="test-result-stat__hint">
                        {{ $isQuestionnaire ? 'Итоговый балл' : 'Клиническая оценка ответа' }}
                    </div>
                </div>
            </section>
            ...
            @if($isImageTest)
                <div class="test-result-layout">
                    <aside class="test-result-sidebar">
                        <section class="test-result-panel">
                            <div class="test-result-panel__header">
                                <h2 class="test-result-panel__title">О результате</h2>
                            </div>
                            <div class="test-result-panel__body">
                                <div class="test-result-note">
                                    Ответы в проективном тесте не должны автоматически трактоваться как диагноз.
                                    Результат предназначен для клинической оценки специалистом с учётом беседы и анамнеза.
                                </div>
                            </div>
                        </section>

                        @if($manualInterpretation !== '')
                            <section class="test-result-panel">
                                <div class="test-result-panel__header">
                                    <h2 class="test-result-panel__title">Комментарий к интерпретации</h2>
                                </div>
                                <div class="test-result-panel__body test-result-interpretation">
                                    {!! nl2br(e($manualInterpretation)) !!}
                                </div>
                            </section>
                        @endif
                        ...
                        @if(!empty($test->instructions))
                            <section class="test-result-panel">
                                <div class="test-result-panel__header">
                                    <h2 class="test-result-panel__title">Инструкция пациенту</h2>
                                </div>
                                <div class="test-result-panel__body test-result-interpretation">
                                    {{ $test->instructions }}
                                </div>
            ...
            @elseif($isCardSort)
                <section class="test-result-panel">
                    <div class="test-result-panel__header">
                        <h2 class="test-result-panel__title">Итоговый порядок карточек</h2>
                ...
                <div class="test-result-sort-info-grid">
                    <section class="test-result-panel">
                        <div class="test-result-panel__header">
                            <h2 class="test-result-panel__title">О результате</h2>
                        </div>
                        ...
                        <div class="test-result-panel__body">
                            <div class="test-result-note">
                                Результат отражает субъективный порядок карточек пациента по заданной шкале.
                                Автоматический клинический вывод формируется только при наличии отдельного
                ...
                <div class="test-result-layout">
                    <aside class="test-result-sidebar">
                        @if($showChart)
                            <section class="test-result-panel">
                                <div class="test-result-panel__header">
                                    <h2 class="test-result-panel__title">Шкалы</h2>
                                </div>
                                <div class="test-result-panel__body">
                                    <canvas id="scalesPie" height="250"></canvas>
                                </div>
                            </section>
                        @endif
                        ...
                        <section class="test-result-panel">
                            <div class="test-result-panel__header">
                                <h2 class="test-result-panel__title">Общая оценка</h2>
                            ...
                            <div class="test-result-panel__body">
                                @if(!is_null($overviewPercent))
                                    <div class="test-result-score">{{ $overviewPercent }}%</div>
                                    <div class="test-result-progress mt-3">
                                        <div class="test-result-progress__bar"
                                             style="width: {{ max(0, min(100, (float) $overviewPercent)) }}%"></div>
                                    </div>
                                    <div class="test-result-description mt-2">
                                        Доля от максимального значения шкалы
                                    </div>
                                @else
                                    <div class="test-result-note test-result-note--warning">
                                        Максимум шкалы не определён, поэтому процент не рассчитан.
                                    </div>
                                @endif
                            </div>
                        </section>
                        ...
                        <section class="test-result-panel">
                            <div class="test-result-panel__header">
                                <h2 class="test-result-panel__title">Итоги по шкалам</h2>
                                <span class="test-result-count">{{ $resultCards->count() }}</span>
                            </div>

                            <div class="test-result-panel__body">
                                @forelse($resultCards as $row)
                                    <div class="test-result-scale-row">
                                        <div class="test-result-scale-row__top">
                                            <div>
                                                <div class="test-result-rank-title">
                                                    {{ data_get($row, 'label', 'Шкала') }}
                                                </div>
                                                @if(!empty(data_get($row, 'range_text')))
                                                    <div class="test-result-rank-subtitle">
                                                        Диапазон: {{ data_get($row, 'range_text') }}
                                                    </div>
                                                @endif
                                            </div>
                                            <div class="test-result-score" style="font-size: 22px;">
                                                {{ data_get($row, 'score', 0) }}
                                            </div>
                                        </div>

                                        @if(!is_null(data_get($row, 'percent')))
                                            <div class="test-result-progress mt-2">
                                                <div class="test-result-progress__bar"
                                                     style="width: {{ max(0, min(100, (float) data_get($row, 'percent'))) }}%"></div>
                                            </div>
                                        @endif
                                    </div>
                                @empty
                                    <div class="test-result-empty">
                                        Результаты по шкалам не сформированы.
                                    </div>
                                @endforelse
                            </div>
                        </section>
                        ...
                        <section class="test-result-panel">
                            <div class="test-result-panel__header">
                                <h2 class="test-result-panel__title">
                                    {{ data_get($interpretation, 'title', 'Интерпретация') }}
                                </h2>

                                @if(!empty(data_get($interpretation, 'badge.text')))
                                    <span class="test-result-status test-result-status--info">
                                        {{ data_get($interpretation, 'badge.text') }}
                                    </span>
                                @endif
                            </div>

                            <div class="test-result-panel__body test-result-interpretation">
                                @if(!empty(data_get($interpretation, 'text')))
                                    <div>
                                        {!! nl2br(e((string) data_get($interpretation, 'text'))) !!}
                                    </div>
                                @else
                                    <div class="test-result-note">
                                        Текстовая интерпретация для этого результата не сформирована.
                                    </div>
                                @endif

                                @if(!empty(data_get($interpretation, 'items')))
                                    <ul>
                                        @foreach((array) data_get($interpretation, 'items') as $item)
                                            <li>{{ $item }}</li>
                                        @endforeach
                                    </ul>
                                @endif
    ...
    @if($showChart)
        ...
        <script>
            (function () {
                ...
                const canvas = document.getElementById('scalesPie');

                if (!canvas || !Array.isArray(source) || source.length === 0 || typeof Chart === 'undefined') {
                    return;
                }
                ...
                new Chart(canvas, {
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            backgroundColor: source.map((item, index) => palette[index % palette.length]),
                            borderColor: '#ffffff',
                            borderWidth: 3,
                            hoverOffset: 5
                        }]
